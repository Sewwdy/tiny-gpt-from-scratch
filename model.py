...
def softmax_overflow_demo(large_value):
    ...
    return {'naive_exp': naive_exp, 'overflowed': np.equal(naive_exp, np.inf)}
...
def stable_softmax_1d(logits):
    ...
    m = max_along_axis(logits, axis=0)
...
def stack_x_batch(data, offsets, block_size):
    ...
    return np.stack([slice_x_at_offset(data, o, block_size) for o in offsets])
...
def generate_sequence(p_matrix, start_id, length, rng):
    ...
    for t in range(1, length):
        seq[t] = sample_next_token(P, seq[t - 1], rng)
    return seq
...
def average_nll(p_matrix, data):
    ...
    s = sum_negative_log_probs(p_matrix, data)
    return s/(data.shape[0]-1)
...
def observe_lookup_equivalence(w, ids):
    onehot = np.zeros((len(ids), w.shape[0]))
...
def cross_entropy_loss(probs, targets):
    ...
    return float(-np.mean(array_log(correct_probs)))
...
def derive_dlogits_on_paper():
    ...
    return 'dL/dlogits = (probs - onehot(targets)) / B. softmax awawawawawwawaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa'
